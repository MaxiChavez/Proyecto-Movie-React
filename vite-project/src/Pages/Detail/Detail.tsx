              ...
              <div className="container">
                <div className="row card-detail">
                  ...
                  <div className="col-lg-7 detail-info">
                    <h2 className='detail-title'>{movie?.title}</h2><br />
                    <h5>Lanzamiento: {convertirFecha(movie?.release_date)}</h5><br />

                    {movie?.overview ? (<><h5>Sinopsis:</h5>
                      <p className='overview-detail'>
                        {movie?.overview}
                      </p><br /></>) : (<></>)}

                    {movie?.genres ? (<><h5 className='grisado'>Generos:
                      {movie?.genres.map((genre) => (
                        <>{" " + genre.name}</>
                      ))}
                    </h5><br /></>) : (<></>)}

                    {movie?.budget != "0" ? (<><h5 className='grisado'>Presupuesto: ${movie?.budget}</h5><br /></>) : (<></>)}

                    {movie?.production_companies ? (<>
                      <h5 className='grisado'>Productoras:
